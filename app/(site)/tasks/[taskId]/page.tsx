...
export default async function TaskPage({ params }: { params: Promise<{ taskId: string }> }) {
  ...
  // способом стереть неудачную попытку. Служебным командам отказ
  // не нужен: у альфы рука заморожена и менять карточку не на
  // что, у проверки контента на руке весь пул. Сервер проверяет
  // всё это ещё раз — здесь мы решаем только, показывать ли
  // кнопку.
  const canDecline =
    eventLive && !session.team.is_test && state === 'available' && latestSubmission === null;
