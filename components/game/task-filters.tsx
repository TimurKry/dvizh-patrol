  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <label className="flex items-center gap-2 text-caption text-stone">
          Сортировка
          <select
            value={sort}
            onChange={(e) => setParam('sort', e.target.value)}
            className="min-h-[40px] rounded-[12px] border border-hairline bg-paper-white px-3 text-caption"
          >
            {SORTS.map((item) => (
