# Миграции 0008–0012: накатывание и откат

Пять миграций, добавленных при доводке до V3 Mono Signal. Здесь
описано, что каждая делает, чем рискует, как её накатить и как
откатить.

Порядок отката — обратный порядку накатывания: `0012`, `0011`,
...
Все пять уже применены к `mnoytybnurzsbvhyuuxb` 11.08.2026, база и
код сходятся. Ниже — порядок на случай второго контура: тестового
