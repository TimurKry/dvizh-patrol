# Откат миграций 0008–0010

Три миграции, добавленные при доводке до V3 Mono Signal. Здесь
описано, что каждая делает, чем рискует и как её откатить.

Порядок отката — обратный порядку накатывания: `0010`, `0009`,
`0008`. Пропускать шаги нельзя: `0009` создаёт тип, на который
ссылается колонка, а `0010` — ограничение, которое проверяет
колонку из той же миграции.
