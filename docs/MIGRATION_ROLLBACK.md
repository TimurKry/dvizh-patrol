# Миграции 0008–0015: накатывание и откат

Восемь миграций, добавленных при доводке до V3 Mono Signal. Здесь
описано, что каждая делает, чем рискует, как её накатить и как
откатить.

Порядок отката — обратный порядку накатывания: `0015`, `0014`, `0013`,
`0012`, `0011`, `0010`, `0009`, `0008`. Пропускать шаги нельзя: `0009` создаёт тип, на
...
Все восемь применены к `mnoytybnurzsbvhyuuxb`: `0008`–`0012` 11.08.2026,
`0013`–`0015` — тогда же, с подтверждения владельца. Ниже — порядок
