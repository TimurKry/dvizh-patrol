# Миграции 0008–0016: накатывание и откат

Девять миграций, добавленных при доводке до V3 Mono Signal. Здесь
описано, что каждая делает, чем рискует, как её накатить и как
откатить.

Порядок отката — обратный порядку накатывания: `0016`, `0015`, `0014`,
`0013`, `0012`, `0011`, `0010`, `0009`, `0008`. Пропускать шаги нельзя: `0009` создаёт тип, на
...
Все девять применены к `mnoytybnurzsbvhyuuxb`: `0008`–`0012` 11.08.2026,
`0013`–`0016` — тогда же, с подтверждения владельца. Ниже — порядок
