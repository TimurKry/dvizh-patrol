# Миграции 0008–0021: накатывание и откат

Четырнадцать миграций, добавленных при доводке до V3 Mono Signal. Здесь
описано, что каждая делает, чем рискует, как её накатить и как
откатить.

Порядок отката — обратный порядку накатывания: `0021`, `0020`, `0019`, `0018`, `0017`, `0016`, `0015`,
...
Все четырнадцать применены к `mnoytybnurzsbvhyuuxb`: `0008`–`0012` 11.08.2026,
`0013`–`0017` — тогда же, `0018`–`0020` — 23.08.2026, `0021` —
25.08.2026, все с подтверждения владельца. Ниже — порядок
...
supabase/migrations/0020_test_team.sql
supabase/migrations/0021_hidden_criteria.sql
```
...
триггер `teams_assign_color` на вставке. Палитра расширена до
десяти цветов в `0029`; предел мероприятия — тоже десять команд,
поэтому цвета хватает всем. Если предел когда-нибудь поднимут,
одиннадцатая команда останется с `color IS NULL` и нейтральной
рубашкой — это ожидаемо, не ошибка, но цвет придётся добавить.
...
## 0021 — скрытые критерии

**Делает:** добавляет `tasks.hidden_criteria` (jsonb-массив строк,
до двенадцати). Открытые критерии читает команда, скрытые уходят
только в запрос к модели.

```sql
ALTER TABLE public.tasks DROP CONSTRAINT IF EXISTS tasks_hidden_criteria_shape;
ALTER TABLE public.tasks DROP COLUMN IF EXISTS hidden_criteria;
```

**Безопасна к откату:** теряются написанные скрытые критерии.
Загадки, переведённые на автоматическую проверку, останутся в
режиме `ai` без единого критерия — модель будет судить по одному
описанию и почти всё отправлять человеку. Перед откатом верните им
...
**Делает:** добавляет `teams.is_test` и `submissions.is_test`,
пересобирает частичный индекс «одна принятая отправка на задание»
с оговоркой `AND NOT is_test`, убирает тестовые команды из
`leaderboard` и правит четыре функции: `join_team` (вход в любом
статусе), `refill_team_hand` (весь пул на руки),
`create_submission_slot` (отправка вне статуса и вне срока, плюс
копия признака в строку) и `accept_submission` (без глобального
захвата).

```sql
-- Порядок обратный: сначала функции и индекс, потом колонки.
-- Определения четырёх функций взять из 0019, 0018 и 0009.
DROP INDEX IF EXISTS public.submissions_one_global_accept_key;
CREATE UNIQUE INDEX submissions_one_global_accept_key
  ON public.submissions (task_id) WHERE status = 'accepted';
ALTER TABLE public.submissions DROP COLUMN IF EXISTS is_test;
ALTER TABLE public.teams DROP COLUMN IF EXISTS is_test;
```

Представления `team_scores` и `leaderboard` тоже придётся вернуть к
версии из `0003_views.sql`: в них появилась колонка `is_test` и
условие по ней.

**Осторожно.** Если в базе есть принятые тестовые отправки, старый
индекс на них упадёт: два принятых по одному заданию — ровно то,
что он запрещает. Перед откатом удалите тестовые команды (это же
вернёт задания в пул).
