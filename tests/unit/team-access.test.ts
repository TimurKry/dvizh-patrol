...
/**
 * Правила, которые держатся не здесь, а в SQL, — но забыть про них
 * легко, поэтому они записаны рядом с режимами.
 *
 * Проверяются на стенде запросами из `docs/`:
 *
 * - у альфы рука раздаётся один раз и не меняется: ни пополнения,
 *   ни снятия отыгравших. Зачтённая карточка остаётся на месте с
 *   пометкой;
 * - зачёт альфы не выносит задание из общего пула;
 * - отказ от карточки служебным командам недоступен: замороженной
 *   руке менять карту не на что;
 * - возврат в обычный режим удаляет служебные отправки команды и
 *   начисления по ним. Иначе набранное на тесте всплывёт в
 *   рейтинге, а потраченные попытки будут считаться на квесте.
 */
describe('плашка состояния', () => {
  it('у служебных режимов объяснение не совпадает с обычным', () => {
    // Обычной команде плашки нет вовсе — это и есть «доступно»,
    // которое убрали с карточек: недоступных карточек на руке не
    // бывает, они с неё уходят.
    expect(teamAccessSpec('normal').player).toBeNull();
    expect(teamAccessSpec('alpha').player).not.toBe(teamAccessSpec('preview').player);
  });
});
